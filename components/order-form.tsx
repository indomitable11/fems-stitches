...
export function OrderForm() {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      {/* Stepper */}
      <ol className="mb-10 flex items-center gap-2" aria-label="Order progress">
        {steps.map((label, i) => {
      ...
      <div className="rounded-lg border border-border bg-card p-6 md:p-8">
        ...
        {step === 0 && (
          <div>
            <h2 className="font-serif text-2xl">What are we making?</h2>
            <p className="mt-1 text-sm text-muted-foreground">Choose the type of outfit you have in mind.</p>
        ...
        {step === 1 && (
          <div className="flex flex-col gap-5">
            <div>
              <h2 className="font-serif text-2xl">Tell us the details</h2>
              <p className="mt-1 text-sm text-muted-foreground">The more you share, the better we can help.</p>
            </div>
            <div>
            ...
            <div>
              <span className={labelClass}>Will you provide your own fabric?</span>
              <div className="flex flex-wrap gap-2">
                {(['Yes', 'No', 'Not sure'] as const).map((opt) => (
                  <button
                    key={opt}
                    type="button"
                    onClick={() => set('ownFabric', opt)}
                    className={cn(
                      'rounded-full border px-3.5 py-1.5 text-sm transition-colors',
                      form.ownFabric === opt
                        ? 'border-foreground bg-foreground text-background'
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label htmlFor="timeline" className={labelClass}>
                  Needed by <span className="text-muted-foreground">(optional)</span>
                </label>
                <input
                  id="timeline"
                  className={inputClass}
          ...
          <div className="flex flex-col gap-5">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Not sure of your measurements? Leave this blank — we&apos;ll help you take them in the
                studio or guide you at home.
              </p>
            </div>
            <div>
              <label htmlFor="measurements" className={labelClass}>
                Measurements <span className="text-muted-foreground">(optional)</span>
              </label>
              <textarea
                id="measurements"
                rows={4}
            ...
            <p className="text-sm text-muted-foreground">
              You can also send inspiration photos directly on WhatsApp after you send this request.
            </p>
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            <h2 className="font-serif text-2xl">Review &amp; send</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              This is the message we&apos;ll open in WhatsApp. You can edit it there before sending.
            </p>
            ...
            <Link
              ...
              className={cn(ctaVariants({ variant: 'whatsapp', size: 'full' }), 'mt-6')}
            >
              <WhatsAppIcon className="size-5" />
              Send on WhatsApp
            </Link>
            <p className="mt-3 text-center text-xs text-muted-foreground">
              No account needed. This opens WhatsApp with your details pre-filled.
            </p>
          </div>
        )}
        ...
        {step < 3 && (
          <div className="mt-8 flex items-center justify-between gap-3">
            <button
              ...
              className={cn(ctaVariants({ variant: 'ghost', size: 'default' }), step === 0 && 'invisible')}
            >
              Back
            </button>
            <button
              ...
              className={ctaVariants({ variant: 'primary', size: 'default' })}
            >
              Continue
            </button>
          </div>
        ...
        {step === 3 && (
          <div className="mt-6">
            <button
              type="button"
              onClick={() => setStep(2)}
              className={ctaVariants({ variant: 'ghost', size: 'default' })}
            >
              Back
            </button>
          </div>
